...
def send_welcome(message):
    ...
    if chat_id in AID:
        ...
        bot.reply_to(message, "Привет! Я бот для хранения учебных материалов. Используй /list для просмотра всех "
                              "материалов, и /search для поиска материалов.")
...
def choose_categories(message):
    ...
    if chat_id in AID:
        ...
        bot.reply_to(message, "Эта функция доступна только администраторам.")
    ...
    if message.content_type == 'document':
        ...
        if not user_data[chat_id]['files']:
            ...
            for media in user_data[chat_id]['files']:
                cursor.execute("INSERT INTO files (material_id, file_id, user_id) VALUES (?, ?, ?)",
                               (material_id, media.media, chat_id))
